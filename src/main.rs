...
fn plot_svg_line(f: &mut File, l: &image::Line, style: &str) -> Result<(), std::io::Error> {
  let (p1, p2) = *l;
...
fn main() {
  ...
  let (left_edge_line_top, left_edge_line_bottom) = box_finder::edge_finder(&in_image, &left_middle, image::Direction::Right);
  let (right_edge_line_bottom, right_edge_line_top) = box_finder::edge_finder(&in_image, &right_middle, image::Direction::Left);
  let (top_edge_line_right, top_edge_line_left) = box_finder::edge_finder(&in_image, &top_middle, image::Direction::Down);
  let (bottom_edge_line_left, bottom_edge_line_right) = box_finder::edge_finder(&in_image, &bottom_middle, image::Direction::Up);
